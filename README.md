...
```
[2026-04-22 10:41:16] [ Main ] Why can't scripts wait until conversation turn ends - prompt: 35035, response: 196, intermediate_prompt: 107377, intermediate_response: 196.
...
| `intermediate_prompt` | Input tokens across all middle API calls — context reloaded once per tool call; this is the expensive part |
| `intermediate_response` | Output tokens across all middle API calls — tool call generation cost; usually small |

A turn with no tool calls has `intermediate_prompt: 0, intermediate_response: 0`. A turn with N tool calls pays approximately N× the context size in `intermediate_prompt`.
...
skill/
  ...
  scripts/
    ...
    count_tokens.py     Session token counter (also lives at ~/.claude/hooks/count_tokens.py)
...
1. Copy the hook scripts:
   ...
   cp skill/scripts/count_tokens.py ~/.claude/hooks/count_tokens.py
   ```
...
```sh
# All entries in the most recent session log
...
# Specific session
uv run --quiet python3 ~/.claude/hooks/count_tokens.py <session_id>
uv run --quiet python3 ~/.claude/hooks/count_tokens.py <session_id> today
```
...
## Why intermediate_prompt dominates
...
- A turn with 5 tool calls pays for 5× the context size in `intermediate_prompt`
